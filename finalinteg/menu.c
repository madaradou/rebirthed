...
void loadMenuSprites(Sprites *Sprites){
    ...
    //bg
    initSpriteHigh(&Sprites->menuMainBg, 5, "./assets/backrounds/menu/main", 500);
    ...
    //ticbtn
    initSprite(&Sprites->tic_Btn, 3, "./assets/sprites/menu/buttons/ptic_btn");
    //resume button
...
void initOptionsMenu(MainMenu *menu, Sprites sprites){
    menu->options_menu.bg = sprites.menuMainBg;
...
void initPLayMenu(MainMenu *menu, Sprites sprites){
    ...
    menu->play_menu.bg = sprites.menuMainBg;
    ...
    initButton(&menu->play_menu.ticBtn, "tic btn", sprites.tic_Btn, 20, 575);
...
// Handle user input
    ...
    if (isMouseInsideButton(&menu->play_menu.loadgameBtn, mouseX, mouseY)){
        ...
        ///tic button
    if (isMouseInsideButton(&menu->play_menu.ticBtn, mouseX, mouseY)){
        if (currentMouseButtonState){
            //click
            menu->play_menu.ticBtn.state = 2;
            SDL_Delay(250);
            playSoundEffect(sounds.btnClickSndEffect);
            *curren_screen = 9;
            ...
            //hover
            if (menu->play_menu.ticBtn.state != 1){
                menu->play_menu.ticBtn.state = 1;
                playSoundEffect(sounds.btnHoverSndEffect);
            }
        }
    }else{
        menu->play_menu.ticBtn.state = 0;
    }
...
void showPlayMenu(MainMenu *menu, SDL_Surface* screen, SDL_Event *event, Sounds sounds, int *quit, int *curren_screen){
    ...
    showButton(menu->play_menu.ticBtn, screen);
