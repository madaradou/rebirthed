...
void initMap(Minimap *m) {
    m->image = SDL_LoadBMP("assets/minimap/backrounds/background_7nayna_3 (1).bmp");
    if (m->image == NULL) {
        printf("Erreur lors du chargement de l'image de la minimap : %s\n", SDL_GetError());
    }
    int x = 200;
    // Ajuster la position de la minimap
   m->pos.x = (1536 - x) /2;//coin superieur droite
   m->pos.y = 10;

}
void initialiser_minimap_perso(Minimap *m, JR *P){
    m->img_bon=IMG_Load("assets/minimap/miniplayer/image1.png");
    m->pos_bon.x=(1536- MINIMAP_WIDTH) +(P->x/scale);
    m->pos_bon.y=(803 - MINIMAP_HEIGHT)+(P->y/scale);
    m->pos_bon.w= P->w/scale;
    m->pos_bon.h= P->h/scale;
}
...
void MAJMinimap(JR player,  Minimap * m, SDL_Rect camera, int redimensionnement){
    ...
    player.y= player.y + camera.y;

    m->pos_bon.x = player.y * redimensionnement/100;
    m->pos_bon.y= player.y * redimensionnement/100;
...
void affichertemps(SDL_Surface* screen, int temps) {
    ...
    TTF_Font* font = TTF_OpenFont("assets/minimap/fonts/oldparisnouveautitling.ttf", 24); 
    ...
    SDL_Rect textPos;
    textPos.x = 50; // Position en X
    textPos.y = 50; // Position en Y
...
SDL_Color GetPixel2(SDL_Surface *pSurface, int x,int y){
 ...
 int CollisionPP(Personne p, SDL_Surface* masque) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        SDL_Color Couleur_Position = GetPixel2(masque, Pos[i].x, Pos[i].y);
